...
export async function get_all_by_room_id(req:any){
    const room_id: number = req.session.room_id;
    const current_time = util.dateToString(new Date());
    const query_room = `SELECT * FROM SEAT WHERE Room_id=${room_id};`;
    const query_reserve = `SELECT Seat_id FROM P_RESERVE WHERE Room_id=${room_id} AND Start_time < '$${current_time}' AND End_time > '${current_time};`;
